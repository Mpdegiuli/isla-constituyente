...
config/idiomas/es.yaml      todo el texto que genera el script (turno, votación, acta), por idioma (es, en, zh, fr)
...
Al 15/9/2026: 80 corridas válidas en `corridas/` (54 de la versión 1 —paneles mono de 12 modelos de 10 laboratorios y tres mesas mixtas: Anthropic ×3, OpenAI, Google, xAI, Mistral, DeepSeek, Alibaba, Moonshot, Zhipu, MiniMax; castellano, inglés y una corrida en chino— y 26 de la versión 2: mono, base y horizonte, diez casas, y seis mesas mixtas —castellano con Sonnet, inglés con Opus, castellano con Fable, "los más antiguos": la versión más vieja que sirve cada casa, de Claude 3 Haiku a Grok 4.20, francés con Opus abriendo, y "los últimos": GPT-6 Astra y Fable 5.1 con los asientos sorteados—; variantes de escasez, abundancia, otra población y horizonte) y 43 inválidas o parciales en `corridas_invalidas/`, cada una con su motivo. Lo leído hasta ahora, en `resultados/`: `cuadro_20260913.md` (las ocho casas iniciales, por corrida y por eje), `convergencia_20260914.md` (tres corridas base por casa), `cuadro_chinas_20260914.md` (Qwen, Kimi, GLM, MiniMax vía OpenRouter), `mixta_20260914.md` (mesas mixtas a ciegas: siete casas, una por asiento; en castellano con Sonnet y en inglés con Opus), `sondeo_identidad_20260914.md` (qué modelo dice ser cada casa y si reconoce la mesa mono), `idioma_20260913.md` (castellano contra inglés), `chino_20260914.md` (Opus 5 con el escenario en chino: idioma de respuesta, tope por carácter y acta por remisión) `v2_20260914.md` (versión 2: el nombre del régimen que eligió cada casa, previsión y seguridad, la otra orilla, procedimiento contra la v1) `mixta_v2_20260915.md` (mesas mixtas con reglas v2, castellano con Sonnet, inglés con Opus y castellano con Fable: quién bautiza, el rol contra la casa, sondeo) `antiguos_20260915.md` (la misma mesa con la versión más vieja de cada casa: casa o generación; `catalogo_versiones_<fecha>.md` es el listado de versiones que sirve cada API, generado por `catalogo_versiones.py`) `frances_20260915.md` (la mesa mixta en francés con Opus abriendo: idioma, quién bautiza, el rol de Mistral, sondeo) y `ultimos_20260915.md` (GPT-6 Astra y Fable 5.1 con los asientos sorteados: qué se queda en el asiento y qué viaja con la casa; la deportista vota no con cualquier modelo) y `codificacion_20260915.md` (las 80 actas en el libro de códigos: eje colectivo–individual por casa, idioma, versión y variante; protección ambiental; la hipótesis del idioma contra la tabla). Cada corrida se contrasta con la predicción escrita antes en `predicciones.md`. Todo lo anterior es la versión 1 del escenario; la versión 2 (`escenario_v2.md`, `agenda: v2`: régimen con nombre como primer punto, previsión y seguridad, relación con la otra orilla cuando la variante está activa, y 500 palabras para el texto del acta; DISENO.md, sección 15) se corre desde el 14/9/2026 con `config/corridas/v2_*.yaml`. La codificación con `codificar.py` se corrió el 15/9/2026 sobre las 80 actas con dos codificadores de casas distintas, Opus 5 (`resultados/codificacion.csv`, `corridas/<id>/codificacion.json`) y GPT-5.5 (`codificacion_gpt55.*`), cada valor con la cita del acta que lo sostiene; la concordancia entre los dos está en `resultados/concordancia_opus_gpt55_20260915-195352.md` (77,8 % de acuerdo exacto, correlación 0,81 entre los índices, kappa sustancial en casi todo y flojo en principio distributivo, resolución de disputas y regla de decisión, por valores que le faltan a la lista) y la lectura en `resultados/codificacion_20260915.md`: con los dos codificadores, en 80 actas no hay ningún mercado, ningún líder único ni ningún castigo físico; Opus es la única casa con freno individual explícito; Gemini es la única con actas "colectivo"; y la hipótesis de que el castellano da actas más colectivistas que el inglés no se sostiene (la misma casa en inglés sale igual o más colectiva). La fórmula distributiva dominante es "según capacidad, según necesidad, con piso" (la de Marx), agregada al libro de códigos como valor para la tercera pasada, en la que los dos codificadores corrieron otra vez con el libro corregido (`codificacion_*_libro2.*`, `concordancia_opus_libro2_gpt55_libro2_*.md`; sección 9 de la lectura): acuerdo 83,4 %, correlación 0,89, disputas y medicamentos arregladas, principio distributivo a medias, y un error de instrumento encontrado ahí: las glosas de los valores del libro estaban en comentarios del yaml y nunca llegaron al codificador. La cuarta pasada (16/9, `codificacion_*_libro3.*`; sección 10) manda cada valor con su glosa en el prompt: acuerdo 88,0 %, la regla de decisión pasa de kappa 0,48 a 0,91 (la regla de dos umbrales es el procedimiento de siete de las nueve mesas mixtas y no aparece en Gemini), el principio distributivo a 0,66 (la fórmula de Marx en 36 y 35 actas; Frohlich–Oppenheimer, 0 y 0; Fable con su propia fórmula, piso igual más plus por trabajo), y tres glosas mal escritas aplanaron forma de gobierno, bote y el casillero `colectivo`, declarado. N corridas por condición: se declara.
